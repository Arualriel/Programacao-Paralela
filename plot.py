...
for k in range(5,9):
    ...
    for i in range(1,5):
       for j in range(6,16):
           N=j*10**5
           values = np.loadtxt("results/results"+str(k)+"/naoOtimizado"+str(i)+str(N)+".txt")
           vec2[j-6] = np.mean(values)
           matrix2[i-1,j-6]=vec2[j-6]
       plt.plot(vec2,label="Original"+str(i),ls='-.')
       #print("***1****",vec2)
    plt.legend()
    ...
    for i in range(1,5):
        for j in range(6,16):
            ...
            values = np.loadtxt("results/results"+str(k)+"/naoOtimizado"+str(i)+str(N)+".txt")
            vec2[j-6] = np.mean(values)
            matrix2[i+3,j-6]=vec2[j-6]
        plt.plot(vec2,label="Original"+str(i),ls='-.')
        #print("***2*****",vec2)
        
       
    plt.legend()
    plt.savefig('codigo'+str(k)+'2.png', format='png')
